...
class Post_Type {
	 ...
	 * @var array
	 */
	public $existingTaxonomies = array();
	 ...
	 */
	public function __construct( $names, $options = array() ) {
		...
		foreach ( $required as $key ) {
			...
			if ( ! isset( $names[ $key ] ) ) {
				...
				if ( $key === 'singular' || $key === 'plural' ) {
					$name = ucwords( strtolower( str_replace( array( '-', '_' ), ' ', $this->postTypeName ) ) );
				...
				// If the slug, slugify the post type name.
					...
					$name = strtolower( str_replace( array( ' ', '_' ), '-', $this->postTypeName ) );
				}
	 ...
	 */
	protected function taxonomy( $names, $options = array() ) {
		// If only the name is passed, create an array.
		if ( ! is_array( $names ) ) {
			$names = array( 'name' => sanitize_title( $names ) );
		}
	...
	public function modifyColumns( $columns ) {
		...
		foreach ( $this->columns()->add as $key => $label ) {
			...
			if ( isset( $this->columns()->positions[ $key ] ) ) {
				...
				// $columns = $start + array($key => $label) + $end;g
				$columns = array_merge( $start, array( $key => $label ), $end );
			} else {
				$columns[ $key ] = $label;
			}
		}
				...
				if ( ! empty( $terms ) ) {
					...
					foreach ( $terms as $term ) {
						...
						$output[] = sprintf(
							...
							esc_url( add_query_arg( array( 'post_type' => $post->post_type, $column => $term->slug ), 'edit.php' ) ), // Create filter url
